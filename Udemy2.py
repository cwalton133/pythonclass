...
## PYTHON 3 BEGINNER'S CHEAT SHEET -   Python 3 - Basics
#Defining a variable
...
b = "$$$Cisco Switch$$$"
print(b.strip("$"))#remove a certain character

#Strings - removing all occurences of a character from a string
a = " Cisco Switch "
print(a.replace(" ", ""))#replace each space character with the absence of any character
...
string1[5:15] #slice starting at index 5 up to, but NOT including, index 15; so index 14
represents the last element in the slice
...
## 10 PYTHON 3 BEGINNER'S CHEAT SHEET
...
## 11 PYTHON 3 BEGINNER'S CHEAT SHEET
...
(1 == 1) and (2 == 2) #result is True; AND means that both operands should be True in order
to get the expression evaluated as True
(1 == 1) or (2 == 2) #result is True; when using OR, it is enough if only one expression is
True, in order to have True as the final result
not(1 == 1) #result is False; using the NOT operator means denying an expression, in this
case denying a True expression
not(1 == 2) #result is True; using the NOT operator means denying an expression, in this case
denying a False expression
None, 0, 0.0, 0L, 0j, empty string, empty list, empty tuple, empty dictionary #these values
...
## 12 PYTHON 3 BEGINNER'S CHEAT SHEET
...
time
sorted(list2, reverse = True) #sorts the elements of a list in descending order and creates a
new list at the same time
list1 + list2 #concatenating two lists
list1 * 3 #repetition of a list
#Lists - slicing (works the same as string slicing, but with list elements instead of string
characters)
a_list[5:15] #slice starting at index 5 up to, but NOT including, index 15; so index 14
represents the last element in the slice
...
## 13 PYTHON 3 BEGINNER'S CHEAT SHEET
...
set1 = set(list1) #creating a set from a list; removing duplicate elements; returns {11, 12, 13,
14, 15}
set2 = set(string1) #creating a set from a string; removing duplicate characters; returns {'b',
'a', 'g', 'f', 'c', 'd', 'e'}; remember that sets are UNORDERED collections of elements
...
## 14 PYTHON 3 BEGINNER'S CHEAT SHEET
type(fs1)
...
##15 PYTHON 3 BEGINNER'S CHEAT SHEET
Python 3 - Tuples
#Tuples - immutable lists (their contents cannot be changed by adding, removing or replacing
elements)
...
## 16 PYTHON 3 BEGINNER'S CHEAT SHEET
(vendor, model, ios) = tuple1 #vendor will be mapped to "Cisco" and so are the rest of the
elements with their corresponding values; both tuples should have the same number of
elements
...
Python 3 - Ranges
#Ranges - unlike in Python 2, where the range() function returned a list, in Python 3 it returns
an iterator; cannot be sliced
...
##17 PYTHON 3 BEGINNER'S CHEAT SHEET
...
dict1.items() #returns a list of tuples, each tuple containing the key and value of each dictionary
pair
...
##18 PYTHON 3 BEGINNER'S CHEAT SHEET
...
#If / Elif / Else conditionals - executing code based on one or more conditions being evaluated
as True or False; the "elif" and "else" clauses are optional
x = 5
if x > 5: #if the "x > 5" expression is evaluated as True, the code indented under the "if" clause
gets executed, otherwise the execution jumps to the "elif" clause...
 print("x is greater than 5")
elif x == 5: #...if the "x == 5" expression is evaluated as True, the code indented under the
"elif" clause gets executed, otherwise the execution jumps to the "else" clause
 print("x IS 5")
else: #this covers all situations not covered by the "if" and "elif" clauses; the "else" clause, if
present, is always the last clause in the code block
 print("x is NOT greater than 5" )
#result of the above "if" block


## 19 PYTHON 3 BEGINNER'S CHEAT SHEET
...
## 20 PYTHON 3 BEGINNER'S CHEAT SHEET
...
5 6 7 8 9 10 5 6 7 8 9 10 5 6 7 8 9 10 5 6 7 8 9 10 5 6 7 8 9 10 5 6 7 8 9 10 5 6 7 8 9 10 5
6 7 8 9 10 5 6 7 8 9 10 5 6 7 8 9 10 #result of the above block
for number in range(10):
 if 5 <= number <= 9: #mixed nesting
 print(number)
5 6 7 8 9 #result of the above block

## 21 PYTHON 3 BEGINNER'S CHEAT SHEET
...
## 22 PYTHON 3 BEGINNER'S CHEAT SHEET
...
Outside the nested loop
for i in range(10):
 pass #pass is the equivalent of "do nothing"; it is actually a placeholder for when you just
# want to write a piece of code that you will treat later

## Python 3 - Try / Except / Else / Finally

#Try / Except / Else / Finally - handling an exception when it occurs and telling Python to keep
executing the rest of the lines of code in the program
try:
 print(4/0) #in the "try" clause you insert the code that you think might generate an exception
at some point
except ZeroDivisionError:
 print("Division Error!") #specifying what exception types Python should expect as a
consequence of running the code inside the "try" block and how to handle them
else:
 print("No exceptions raised by the try block!") #executed if the code inside the "try" block
raises NO exceptions
finally:
 print("I don't care if an exception was raised or not!") #executed whether the code inside
the "try" block raises an exception or not
#result of the above block
Division Error!
I don't care if an exception was raised or not!

## 23 PYTHON 3 BEGINNER'S CHEAT SHEET
...
def my_first_function(x, y): #defining a function that takes two parameters
 sum = x + y
 return sum #this statement is used to exit a function and return something when the
function is called

my_first_function(1, 2) #calling a function and passing two POSITIONAL arguments, the
values of 1 and 2; result is 3
my_first_function(x = 1, y = 2) #calling a function and passing two KEYWORD arguments, the
values of 1 and 2; result is 3
my_first_function(1, y = 2) #calling a function and passing mixed types of arguments, the
values of 1 and 2; result is 3; rule: positional arguments always before keyword arguments!

def my_first_function(x, y, z = 3): #specifying a default parameter value in a function definition
def my_first_function(x, *args) #specifying a variable number of positional parameters in a
function definition; args is a tuple
def my_first_function(x, **kwargs) #specifying a variable number of keyword parameters in a
function definition; args is a tuple
global my_var #"importing" a variable in the global namespace to the local namespace of a
function

## 24 PYTHON 3 BEGINNER'S CHEAT SHEET

## Python 3 - Modules

#Modules and importing - Basics
import sys #importing the sys module; the import statements should be placed before any
other code in your application
from math import pi #importing only a variable (pi) from the math module
from math import sin #importing only a function (sin()) from the math module; there's no need
to add the parantheses of the function when importing it
from math import * #importing all the names (variables and functions) from the math module
#Installing a non-default Python 3 module in Windows is done from the command line (e.g.
the openpyxl module)
C:\WINDOWS\system32> pip install openpyx
#Installing a non-default Python 3 module in macOS is done from the terminal (e.g. the
openpyxl module)
mihais-MacBook-Pro:~ mihai$ pip3 install openpyx
Python 3 - File Operations
#Files - opening and reading a file
myfile = open("routers.txt", "r") #"r" is the file access mode for reading and it is the default
...
## 25 PYTHON 3 BEGINNER'S CHEAT SHEET
...
newfile = open("newfile.txt", "w") #opens/creates a new file for writing; the "w" method also
creates the file for writing if the file doesn’t exist and overrides the file if the file already exists;
remember to close the file after writing to it to save the changes!
newfile.writelines(["Cisco", "Juniper", "HP", "\n"]) #this method takes a sequence of strings
as an argument and writes those strings to the file
...
with open("python.txt", "w") as f: #using the with-as solution, the files gets closed
automatically, without needing the close() method
...
## 26 PYTHON 3 BEGINNER'S CHEAT SHEET
...
## 27 PYTHON 3 BEGINNER'S CHEAT SHEET
#Regular Expressions - the "re.findall" and "re.sub" methods
a = re.findall(r"\d\d\.\d{2}\.[0-9][0-9]\.[0-9]{1,3}", arp) #returns a list where each element is
#a pattern that was matched inside the target string
['22.22.22.1'] #result of the above operation - a list with only one element, the IP address
matched by the regex
b = re.sub(r"\d", "7", arp) #replaces all occurrences of the specified pattern in the target string
with a string you enter as an argument
'77.77.77.7 7 b7:a7:7a:ff:c7:77 VLAN#777 L 77.77.77.77' #result of the
above operation
...
class MyRouter(object): #creating a class which inherts from the default "object" class
 def __init__(self, routername, model, serialno, ios): #class constructor; initializing some
#variables and the method is called whenever you create a new instance of the class
...
router1 = MyRouter('R1', '2600', '123456', '12.4') #creating an object by simply calling the
#class name and entering the arguments required by the __init__ method in between parentheses
router1.model #accessing the object's attributes; result is '2600'

## 28 PYTHON 3 BEGINNER'S CHEAT SHEET
...
## 29 PYTHON 3 BEGINNER'S CHEAT SHEET
...
#...we can use a lambda function, a list comprehension and concatenation on a single line of
code
...
32
PYTHON 3 BEGINNER'S CHEAT SHEET
#cycle() - returns an iterator that simply repeats the value given as argument infinitely; you
have to find a way to break out of the infinite loop
a = range(11, 16)
for i in cycle(a):
...
list(filterfalse(lambda x: x < 5, [1, 2, 3, 4, 5, 6, 7])) #in Python 2 the result is [5, 6, 7]; in
Python 3 there is no ifilter() like in Python 2, just filter() and filterfalse()
